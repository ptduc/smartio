...
class MQTT_Client
  def initialize(host = localhost, port = 1883)
    @mqtt_client = MQTT::Client.connect(host, port)
    # @db_conn_string = 'postgres://smartio:smartio@kynguyenxanh.org:5432/smartio_development'
    @db_conn_string = 'postgres://smartio:smartio@kynguyenxanh.org:5432/smartio_production'
    @db_conn = PG::Connection.new(@db_conn_string)
    @db_devices = db_get_list_devices
  end

  def run
    puts "MQTT_Client on running"
